...
def get_translation_dataloaders(data_path: str = './data', 
    ...
    # Try to build vocab compatible with pre-trained model
    if use_pretrained_vocab:
        try:
            # Try to load and inspect the checkpoint to get expected vocab sizes
            checkpoint = torch.load('RNN-TR-model.pt', map_location='cpu')
            expected_src_vocab = checkpoint['encoder.embedding.weight'].shape[0]
            expected_tgt_vocab = checkpoint['decoder.embedding.weight'].shape[0]
            
            print(f"Pre-trained model expects: src_vocab={expected_src_vocab}, tgt_vocab={expected_tgt_vocab}")
            
            # Build vocab with higher min_freq to reduce vocabulary size if needed
            min_freq = 1
            vocabs = build_vocab(train_dataset.data, tokenizers, src_lang, tgt_lang, min_freq)
            
            # If our vocab is larger than expected, increase min_freq
            while (len(vocabs[src_lang]) > expected_src_vocab or 
                   len(vocabs[tgt_lang]) > expected_tgt_vocab) and min_freq < 5:
                min_freq += 1
                print(f"Trying min_freq={min_freq} to match pre-trained vocab size...")
                vocabs = build_vocab(train_dataset.data, tokenizers, src_lang, tgt_lang, min_freq)
                
            print(f"Built vocab with min_freq={min_freq}")
            
        except Exception as e:
            print(f"Could not load pre-trained model for vocab matching: {e}")
            print("Building vocab normally...")
...
def create_model_from_checkpoint(checkpoint_path='RNN-TR-model.pt', device='cpu'):
    """Create a model that exactly matches a pre-trained checkpoint"""
    try:
        checkpoint = torch.load(checkpoint_path, map_location=device)
        
        # Extract dimensions from checkpoint
        encoder_embed_shape = checkpoint['encoder.embedding.weight'].shape
        decoder_embed_shape = checkpoint['decoder.embedding.weight'].shape
        lstm_weight_shape = checkpoint['encoder.lstm.weight_ih_l0'].shape
        fc_weight_shape = checkpoint['decoder.fc_out.weight'].shape
        
        INPUT_DIM = encoder_embed_shape[0]  # source vocab size
        OUTPUT_DIM = decoder_embed_shape[0]  # target vocab size
        ENC_EMB_DIM = encoder_embed_shape[1]  # embedding dim
        DEC_EMB_DIM = decoder_embed_shape[1]  # should be same
        HID_DIM = lstm_weight_shape[0] // 4  # LSTM hidden dim (divided by 4 for gates)
        
        # Try to infer other parameters
        N_LAYERS = 1  # Default, could be extracted from key names
        ENC_DROPOUT = 0.3  # Default
        DEC_DROPOUT = 0.3  # Default
        
        print(f"Creating model to match checkpoint:")
        print(f"  INPUT_DIM (src vocab): {INPUT_DIM}")
        print(f"  OUTPUT_DIM (tgt vocab): {OUTPUT_DIM}")
        ...
        model = Seq2Seq(enc, dec, device, trg_vocab=None)  # We'll set this later
        
        return model, (INPUT_DIM, OUTPUT_DIM, ENC_EMB_DIM, DEC_EMB_DIM, HID_DIM, N_LAYERS)
        
    except Exception as e:
        print(f"Could not create model from checkpoint: {e}")
        return None, None
...
if __name__ == "__main__":
    ...
    # First try to create model that matches checkpoint dimensions
    checkpoint_model, checkpoint_dims = create_model_from_checkpoint('RNN-TR-model.pt', device)
    
    if checkpoint_model is not None:
        print("Using model dimensions from checkpoint for compatibility...")
        train_dataloader, valid_dataloader, vocab_transform = get_translation_dataloaders(
            batch_size=4, use_pretrained_vocab=True)
        
        # Use the model with correct dimensions
        model = checkpoint_model.to(device)
        model.trg_vocab = vocab_transform['en']  # Set the target vocab
        ...
        print("Creating model with current vocabulary...")
        train_dataloader, valid_dataloader, vocab_transform = get_translation_dataloaders(
            batch_size=4, use_pretrained_vocab=False)
    ...
    # Show some example translations (optional, comment out if you want to skip)
    try:
        data_itr = iter(train_dataloader)
        for n in range(min(1000, len(train_dataloader))):  # Skip to longer sequences
            try:
        ...
        print(f"Skipping example display due to: {e}")
    ...
    try:
        checkpoint = torch.load('RNN-TR-model.pt', map_location=device)
        
        # Check if vocabulary sizes match
        encoder_weight_shape = checkpoint['encoder.embedding.weight'].shape
        decoder_weight_shape = checkpoint['decoder.embedding.weight'].shape
        
        expected_src_vocab = encoder_weight_shape[0]
        expected_tgt_vocab = decoder_weight_shape[0]
        actual_src_vocab = len(vocab_transform['de'])
        actual_tgt_vocab = len(vocab_transform['en'])
        
        print(f"Checkpoint expects: src_vocab={expected_src_vocab}, tgt_vocab={expected_tgt_vocab}")
        print(f"Current model has: src_vocab={actual_src_vocab}, tgt_vocab={actual_tgt_vocab}")
        
        if expected_src_vocab == actual_src_vocab and expected_tgt_vocab == actual_tgt_vocab:
            # Perfect match - load normally
            model.load_state_dict(checkpoint)
            print("✓ Loaded pre-trained model weights (exact match)")
        else:
            # Vocabulary size mismatch - load what we can
            print("⚠ Vocabulary size mismatch detected. Loading compatible layers only...")
            ...
            for name, param in checkpoint.items():
                if name in model_dict:
                    if param.shape == model_dict[name].shape:
                        ...
                        print(f"  ✗ Skipped: {name} (shape mismatch)")
                else:
            ...
            print("Note: Embedding layers and output layer will use random initialization")
