...
epochs = 50
sample_interval = 5
learning_rate = 0.0002  # Lower learning rate to prevent too fast learning
...
def build_discriminator():
    ...
    model.add(Dense(512))
    model.add(LeakyReLU(alpha=0.2))
    model.add(Dropout(0.4))  # Add dropout to prevent discriminator from being too powerful
    ...
    # Output layer - sigmoid with label smoothing
...
real = np.ones((batch_size, 1)) * 0.9  # Label smoothing: use 0.9 instead of 1
fake = np.zeros((batch_size, 1)) + 0.1  # Label smoothing: use 0.1 instead of 0
