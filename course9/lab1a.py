...
# Custom tokenizer class to replace torchtext tokenizer
class SimpleTokenizer:
    """A basic English tokenizer that splits on whitespace and removes punctuation."""
...
class SimpleVocab:
    """A simple vocabulary class that maps tokens to indices."""
    ...
    def set_default_index(self, default_idx):
        """Set the default index for unknown tokens."""
        if isinstance(default_idx, str):
            self.default_index = self.stoi.get(default_idx, 0)
        else:
            self.default_index = default_idx
    ...
    def __call__(self, tokens):
        ...
        for token in tokens:
            if token in self.stoi:
                ...
                indices.append(self.default_index if self.default_index is not None else 0)
        return indices
    ...
    def get_itos(self):
        """Get index to string mapping (list of tokens)."""
        return self.itos
...
def build_vocab_from_iterator(token_iterator, specials=None):
    ...
    return SimpleVocab(all_tokens, specials)
...
vocab.set_default_index(vocab.get_stoi()["<unk>"])  # Set default index for unknown words
...
# Create a new 2D figure for plotting positional encodings
