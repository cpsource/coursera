...
model = SoftMax(input_size, output_size).to(device)
...
# Modified training function with CUDA support
def train_model(model, train_loader, validation_loader, criterion, optimizer, epochs, device):
    ...
    for epoch in range(epochs):
        ...
        # Optional: Clear cache if using CUDA to manage memory
        if device.type == 'cuda':
...
training_loss, validation_accuracy = train_model(model, train_loader, validation_loader, criterion, optimizer, epochs, device)
