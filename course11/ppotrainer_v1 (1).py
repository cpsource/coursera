...
Original file is located at
    https://colab.research.google.com/drive/1eSiSY0ohdvuLQCiQj86AI902VryHrS7b
...
base_model = AutoModelForCausalLM.from_pretrained("lvwerra/gpt2-imdb")
...
# Step 1: Specify model name directly
model_name = "lvwerra/gpt2-imdb"  # or your preferred model

# Step 2: Load base model and wrap with value head
...
model_name = "lvwerra/gpt2-imdb"  # or your original config.model_name
...
model_name = "lvwerra/gpt2-imdb"  # or use your own model

# Step 1: Load base model
base_ref_model = AutoModelForCausalLM.from_pretrained(model_name)

# Step 2: Wrap it with a value head
ref_model = AutoModelForCausalLMWithValueHead.from_pretrained(base_ref_model)
...
def build_dataset(model_name, dataset_name="imdb", input_min_text_length=2, input_max_text_length=8):
    """
    Build dataset for training from Hugging Face Datasets.

    Args:
        model_name (str): Model ID for tokenizer.
        dataset_name (str): Name of dataset to load (e.g., "imdb").
        input_min_text_length (int): Minimum number of tokens.
        input_max_text_length (int): Maximum number of tokens.

    Returns:
        torch.utils.data.Dataset: Tokenized and filtered dataset.
    """

    # Load tokenizer manually (no config.model_name)
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    tokenizer.pad_token = tokenizer.eos_token
...
dataset = build_dataset(model_name="lvwerra/gpt2-imdb")
...
ppo_config = PPOConfig(
    model_name=model_name,         # still required by TRL 0.7.x for internal cloning
    learning_rate=1.41e-5,
    batch_size=16,
    mini_batch_size=4,
    gradient_accumulation_steps=1,
...
ppo_trainer = PPOTrainer(
    model=model,
    ref_model=ref_model,           # Optional, can be None
    tokenizer=tokenizer,
    dataset=dataset,
    data_collator=collator,        # Optional, but recommended
    config=ppo_config              # this must be a PPOConfig object
)
...
# 2. Initialize PPOTrainer WITHOUT tokenizer
ppo_trainer = PPOTrainer(
    model=model,
    ref_model=ref_model,
    dataset=dataset,
...
# 3. Assign tokenizer after
ppo_trainer.tokenizer = tokenizer

from trl import PPOTrainer, PPOConfig
from transformers import TrainingArguments

# Legacy TRL PPOConfig
ppo_config = PPOConfig(
    learning_rate=1.41e-5,
    batch_size=16,
    mini_batch_size=4,
